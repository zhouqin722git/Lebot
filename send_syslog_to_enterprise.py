...
def set_ioc_ip(host):
    url = 'http://' + host + ':9100/threat_intelligence_credibility/_search?pretty'
    payload = {"query": {"match": {"type": type}}, "sort": {"timestamp": "desc"}, "size": 1}
    ...
    if response.status_code == 200:
        ...
        return list_ioc
print set_ioc_ip('172.16.100.236')
...
def send_json(raw_log_file, host, iter_times):
    ...
    try:
        ...
        for i in range(int(iter_times)):
            ...
            syslog = "src:1.1.1.1,dst:2.2.2.2,domain:domain,hash_file:hash,url:http"
            print "发送的日志为： ", syslog
            s.sendall(syslog)
